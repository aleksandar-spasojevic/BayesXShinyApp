  ...
  bivnormal = structure(
    list(
      density = function(mu_stunting2, mu_wasting2, 
        ...
        cov[upper.tri(cov)] <- rho_stunting2 * sigma_stunting2 * sigma_wasting2
      ...
      moment = list(
        ...
        var = function(sigma_stunting2, sigma_wasting2, ...) {
          return( sigma_stunting2 )
        },
        
        # correlation function
        cor = function(rho_stunting2, ...){
          return( rho_stunting2 )
        }
      )
    ), class = c("distribution", "list"))
)
...
plot.univariate <- function(density, xlab = "", ...){
  quantiles <- t(apply(density, 1, quantile, c(0.05, 0.95), na.rm = TRUE))
